...
void initialize(std::vector<std::vector<int>>& randomMap, mappedNodes& knownNodes, priorityList& uList, Node startNode, Node goalNode){
    ...
    uList[calculateKey(goalNode, startNode, uList)] = knownNodes[goalNode.coord];
        ...
        for (int j = node.coord.second-1 ; j <= node.coord.second +1; j++)
        {
            ...
            if(knownNodes.at(nodeCoord).isObstacle){
                ...
                if(knownNodes.at(nodeCoord).isObstacle || node.isObstacle){
                    ...
                    rhsValues.push_back(nodeCost*sqrt(distance2(node,knownNodes.at(nodeCoord))) +knownNodes.at(nodeCoord).costG);
                }
                if(rhsValues.back()<0){
