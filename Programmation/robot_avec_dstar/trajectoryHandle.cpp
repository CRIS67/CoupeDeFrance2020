...
bool detectCollision(std::vector<std::vector<int>> &map, std::vector<Node> path, Node start, Node nextDestination)
{
	...
	// std::cout<<start.coord.first<<"  "<<start.coord.second<<std::endl;
	// std::cout<<path.at(indexOfNextDestination).coord.first<<"  "<<path.at(indexOfNextDestination).coord.second<<std::endl;
	// std::cout<<nextDestination.coord.first<<"  "<<nextDestination.coord.second<<std::endl;


	if (detectCollisionLine(start.coord.first, start.coord.second, nextDestination.coord.first, nextDestination.coord.second, map)){
		// std::cout<<"ici0"<<std::endl;
		return true;
	}

	// std::cout<<"path size"<<pathSize<<std::endl;
	// std::cout<<"next"<<indexOfNextDestination<<std::endl;
	for (unsigned int i = indexOfNextDestination; i < path.size() - 1; i++)
	{
		...
		if (detectCollisionLine(path.at(i).coord.first, path.at(i).coord.second, path.at(i + 1).coord.first, path.at(i + 1).coord.second, map)){
			//std::cout<<"ici"<<(i-indexOfNextDestination+1)<<std::endl;
			return true;
		}
...
bool detectCollisionLine(float x1, float y1, float x2, float y2, std::vector<std::vector<int>> &mapVector)
{
	...
	do{
		e2 = err << 1;
		//std::cout<<"x="<<x<<"  y="<<y<<" value="<<mapVector[y][x]<<" "<<e2<<"  "<<err<<"  "<<dx<<"  "<<dy<<std::endl;
		if (mapVector[y][x] == 1) { return true; }
		if (e2 > dy) {
			err += dy;
			x += sx;
		}
