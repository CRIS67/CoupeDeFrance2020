...
int DynamixelClass::moveSpeed(unsigned char ID, int Position, int Speed)
{
    ...
    Position_L = Position;                // Voir datasheet : 16 bits - 2 x 8 bits variables
    Speed_H = Speed >> 8;
    Speed_L = Speed;                      // Idem : 16 bits - 2 x 8 bits variables
	Checksum = (~(ID + AX_GOAL_SP_LENGTH + AX_WRITE_DATA + AX_GOAL_POSITION_L + Position_L + Position_H + Speed_L + Speed_H))&0xFF;
 
	switchCom(Direction_Pin,Tx_MODE);    // Pour gérer le pin du buffer
    sendData(AX_START);                // Envoi des instructions séries, voir .h pour les adresses
    sendData(AX_START);
    sendData(ID);
    ...
    return (read_error());               // En cas d'erreur
}
